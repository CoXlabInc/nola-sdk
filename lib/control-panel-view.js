  ...
  buildProject() {
    ...
    try {
      ...
      atom.notifications.addWarning('No \'Nol.A-project.json\' found!', {
        detail: 'Make sure that the \'Nol.A-project.cpp\' exists in the current project directory.',
      });

      return;
    }

    if (projectInfo.board == undefined) {
    ...
    fs.writeFileSync(this.currentProjectPath + path.sep + 'Nol.A-project.json', JSON.stringify(projectInfo, null, ' '));
