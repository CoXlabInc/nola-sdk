...
export default class ControlPanelView {
  ...
  constructor(serializedState) {
    ...
    atom.config.observe('nola-sdk.libVersions', this.refreshLibVer);
  }
  ...
  buildProject() {
    ...
    if (atom.config.get('nola-sdk.hwLibDevMode') == true) {
      var srcPath = atom.config.get('nola-sdk.hwLibSourcePath');

      try {
        fs.statSync(srcPath);
        fs.statSync(srcPath + path.sep + 'Makefile');
        fs.statSync(srcPath + path.sep + 'build' + path.sep + 'Makefile');
        fs.statSync(srcPath + path.sep + 'build' + path.sep + this.devSelectView.curDev.libType + path.sep + 'Makefile');
      } catch(e) {
        alert('The hardware library source path is not valid.');
        return;
      }
    }
