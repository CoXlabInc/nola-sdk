...
export default class ControlPanelView {
  ...
  constructor(serializedState) {
    ...
    this.textProjectName = document.createElement('code');
    this.textProjectName.textContent = 'Please create or open project.';
    this.element.appendChild(this.textProjectName);
    ...
    this.textProjectName.addEventListener('click', this);
  ...
  handleEvent(event) {
    if (event.target == this.btnNewProject && event.type == 'click') {
    ...
    } else if (event.target == this.textProjectName && this.currentProjectPath != undefined && event.type == 'click') {
      ...
      atom.commands.dispatch(atom.views.getView(atom.workspace), 'nola-sdk:buildProject');
  ...
  setProject(projectPath) {
    if (projectPath != null) {
      ...
      this.textProjectName.textContent = 'Project: ' + path.basename(projectPath);
