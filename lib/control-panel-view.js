...
export default class ControlPanelView {
  ...
  constructor(serializedState) {
    ...
    const boards = atom.config.get('nola-sdk.libVersions');
    if (boards != undefined) {
      const builder = boards.find((element) => { return (element.libType == "builder") });
      if (builder != undefined && builder.libVersion != undefined) {
        this.textDevLibVer.textContent = builder.libVersion;
      }
    }
    ...
    this.btnSettings.classList.add('btn');
    this.btnSettings.classList.add('tool-bar-btn');
    this.btnSettings.classList.add('icon');
    this.btnSettings.classList.add('icon-settings');
    this.element.appendChild(this.btnSettings);
    ...
    this.devSelectView = new DeviceSelectView(this);
    this.portSelectView = new PortSelectView(this);
    ...
    this.btnSettings.addEventListener('click', this);
  ...
  handleEvent(event) {
    if (event.target == this.btnNewProject && event.type == 'click') {
      ...
      this.cleanProject();
    } else if (event.target == this.btnSettings && event.type == 'click') {
      atom.workspace.open('atom://config/packages/nola-sdk')
  ...
  buildProject() {
    ...
    if (typeof this.portSelectView.curPort === 'string') {
      projectInfo.port = this.portSelectView.curPort.trim();
    } else if (typeof this.portSelectView.curPort.comName !== 'undefined') {
      projectInfo.port = this.portSelectView.curPort.comName;
    }
  ...
  cleanProject() {
    ...
    if (atom.config.get('nola-sdk.hwLibDevMode')) {
    ...
    fs.remove(this.currentProjectPath + path.sep + 'build', () => {
      if (hwLibSrcPath == '') {
        ...
        if (os.platform() == 'win32' || os.platform() == 'win64') {
          const libPath = `${hwLibSrcPath}/build/${this.devSelectView.curDev.libType}`;
          spawn('wsl', [ 'rm', '-rf', libPath ]).on('close', (code) => {
            atom.notifications.addInfo("Clean done!", {
              detail: `Project: ${this.currentProjectPath}\nHardware library output files: ${libPath} (in WSL)`,
            });
          });
        } else {
