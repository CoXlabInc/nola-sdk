...
export default class UpdateLibraryView {
  ...
  finishUpdate() {
    ...
    let newList = this.libVersions;

    for (let i in devList) {
      let dev = devList[i];
      if (typeof devList.find((element) => { return (element.libType === dev.libType); }) === "undefined") {
        newList.push(dev);
      }
    }

    atom.config.set('nola-sdk.libVersions', newList);

    let currentTools = [];
    for (let i in newList) {
      if (typeof newList[i].libToolchain !== 'undefined' && !currentTools.includes(newList[i].libToolchain)) {
        currentTools.push(newList[i].libToolchain);
      }
    }
    ...
    let builder = newList.find((element) => {
