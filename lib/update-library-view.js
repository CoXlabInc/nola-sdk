...
export default class UpdateLibraryView {
  ...
  doUpdate() {
    ...
    const serverURL = 'http://download.coxlab.kr';
    ...
    if (libNewVer == 'DELETE') {
      for (j = 0; j < this.libVersions.length; j++) {
        if (this.libVersions[j].libType == libType) {
          this.statusText.textContent = 'Deleting ' + libName + '...';
          this.libVersions.splice(j, 1);
          fs.removeSync(filePath + libType);
          this.statusText.textContent = 'Deleting ' + libName + ' is done.';
          break;
        }
      }
      ...
      this.index++;
      this.prog.value += 2;
      if (this.prog.value == this.prog.max) {
        this.finishUpdate();
      } else {
        this.doUpdate();
      }
          ...
          yazul.open(filePath + fileName, { lazyEntries: true }, (err, zipFile) => {
            ...
            zipFile.on("close", () => {
              ...
              if (view.libVersions.length == 0) {
                ...
                for (j = 0; j < view.libVersions.length; j++) {
                  if (view.libVersions[j].libType == libType) {
                    view.libVersions[j].libVersion = libNewVer;
                    view.libVersions[j].libToolchain = libToolchain;
                    break;
                  }
                }
