...
export default class UpdateLibraryView {
  ...
  finishUpdate() {
    ...
    let devList = atom.config.get('nola-sdk.libVersions');
    let updatedList = this.libVersions;

    //console.log(`devList: ${JSON.stringify(devList)}`);
    //console.log(`updatedList: ${JSON.stringify(updatedList)}`);

    for (let i in devList) {
      let updated = updatedList.find((element) => { return (element.libType === devList[i].libType); });
      if (typeof updated !== 'undefined') {
        devList[i].libVersion = updated.libVersion;
      }
    }
    for (let updated of updatedList) {
      if (devList.findIndex((element) => {return (element.libType === updated.libType); }) < 0) {
        devList.push(updated);
      }
    }

    //console.log(`merged: ${JSON.stringify(devList)}`);
    atom.config.set('nola-sdk.libVersions', devList);

    let currentTools = [];
    for (let dev of devList) {
      if (typeof dev.libToolchain !== 'undefined' && !currentTools.includes(dev.libToolchain)) {
        currentTools.push(dev.libToolchain);
      }
    }

    let existingTools = atom.config.get('nola-sdk.toolchains');
    let removeTools = [];
    for (let i in existingTools) {
      if (!existingTools[i].startsWith('tools-') && !currentTools.includes(existingTools[i])) {
        fs.remove(app.getPath('home') + path.sep + '.atom' + path.sep + 'nola-sdk' + path.sep + 'tools' + path.sep + existingTools[i]);
        removeTools.push(existingTools[i]);
      }
    }

    for (let i in removeTools) {
      existingTools.splice(existingTools.indexOf(removeTools[i]), 1);
    }
    atom.config.set('nola-sdk.toolchains', existingTools);

    this.controlPanel.devSelectView.loadDevice();
    let builder = devList.find((element) => {
      return (element.libType === "builder");
    });
    if (typeof builder !== "undefined") {
      this.controlPanel.textDevLibVer.textContent = builder.libVersion;
    }
  }
