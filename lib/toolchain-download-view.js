...
export default class ToolchainDownloadView {
  ...
  constructor(toolchain, serializedState) {
    this.element = document.createElement('div');
    this.element.classList.add('nola-sdk');

    const title = document.createElement('h1');
    title.textContent = 'Installing the toolchain...';
    ...
    this.targetToolchain = toolchain;
  }
  ...
  doDownload() {
    const installPath = app.getPath('home') + path.sep + '.atom' + path.sep + 'nola-sdk' + path.sep + 'tools';
    fs.ensureDirSync(installPath);

    var url = 'http://coxlab.iptime.org/downloads/toolchains/' + this.targetToolchain + '-';
    ...
    this.request = http.get(url, (response) => {
      ...
      response.on('data', (chunk) => {
        ...
        this.statusText.textContent = (
          'Downloading ' + percent + ' % ('
          + parseInt(this.downloaded / 1024 / 1024) + '/'
          + parseInt(this.totalSize / 1024 / 1024) + 'MB)');
      ...
      file.on('finish', () => {
        ...
        if (this.totalSize != this.downloaded) {
          ...
          this.statusText.textContent = 'Extracting...';
          this.btnCancel.diabled = true;
          this.btnCancel.textContent = 'Wait...';
          ...
          this.zipFile.on('close', () => {
            ...
            if (os.platform() == 'linux' || os.platform() == 'darwin') {
              chmodr.sync(installPath + path.sep + this.targetToolchain, 0o755);
            }

            this.statusText.textContent = 'Completed';
            this.btnCancel.diabled = false;
