...
export default {
      ...
      noti = atom.notifications.addInfo("Update for Nol.A SDK!",
                                        {
                                          ...
                                          buttons: [{
                                            ...
                                            onDidClick() {
                                              noti.dismiss();
                                              updateView = new UpdateLibraryView(
                                                updateList,
                                                sdk.controlPanel
                                              );
                                              updateView.doUpdate();
  ...
  essentialToolsUpdate() {
    ...
    let toolchainList = atom.config.get('nola-sdk.toolchains');
    let toolsToBeRemoved = [];
    let needToUpdate = true;
    for (i in toolchainList) {
      if (toolchainList[i].substring(0, 5) == 'tools') {
        if (toolchainList[i] == 'tools-1.0.0') {
          needToUpdate = false;
        } else {
          toolsToBeRemoved.push(toolchainList[i]);
        }
      }
    }

    for (i in toolsToBeRemoved) {
      toolchainList.splice(toolchainList.indexOf(toolsToBeRemoved[i]), 1);
    }
