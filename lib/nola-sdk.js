...
export default {
  ...
  provideBuilder() {
    ...
    return class NolABuildProvider extends EventEmitter {
      ...
      isEligible() {
        console.log(`isEligible?: cwd(${this.cwd})`);
      ...
      settings() {
        ...
        config.args[0] = app.getPath('home') + path.sep + '.atom';

        var hwLibSourcePath = atom.config.get('nola-sdk.hwLibSourcePath');

        if (atom.config.get('nola-sdk.hwLibDevMode') == true && hwLibSourcePath != '' && path.isAbsolute(hwLibSourcePath)) {
          try {
            fs.statSync(hwLibSourcePath + path.sep + 'Makefile');
            config.args[0] = hwLibSourcePath;
          } catch(e) {
          }
        }

        config.args[0] += path.sep + 'nola-sdk' + path.sep + 'make' + path.sep + 'build.js';

        console.log(`exec:${config.exec},args:${config.args}`);
        return config;
      }
    };
  },
